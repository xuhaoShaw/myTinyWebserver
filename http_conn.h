...
class http_conn {
public:
    static int m_epollFd; //所有socket上事件都被注册到同一个epoll文件描述符中
    static int m_userCnt; //统计用户数量
    ...
    */
    ...
    void init(int sockFd, const sockaddr_in & addr); //初始化新接收的对象
...
private:
    ...
    int m_read_index;          //标志缓冲区中读入客户端数据最后一个字节的下一个位置
